...
集成已将 BD09 坐标自动转换为 WGS84，可直接使用 **HA 内置地图卡片** 显示设备位置。

使用方法：仪表盘 → 添加卡片 → 地图 → 选择你的设备实体即可。

> 坐标已自动转换，在 HA 默认地图 (OSM) 上位置准确。原始 BD09 坐标保存在 entity attributes 的 `bd09_latitude` / `bd09_longitude` 中。
